...
{{ if .IsMaster }}
Environment="K3S_ARGS= {{ range .CertSANs }} --tls-san={{ . }}{{- end }} {{ range .ApiserverArgs }} --kube-apiserver-arg={{ . }}{{- end }} {{ range .ControllerManager }} --kube-controller-manager-arg={{ . }}{{- end }} {{ range .SchedulerArgs }} --kube-scheduler-arg={{ . }}{{- end }} --cluster-cidr={{ .PodSubnet }} --service-cidr={{ .ServiceSubnet }} --cluster-dns={{ .ClusterDns }} --flannel-backend=none --disable-network-policy --disable-cloud-controller --disable=servicelb,traefik,metrics-server,local-storage  --kube-controller-manager-arg=leader-elect-renew-deadline=30s  --kube-controller-manager-arg=leader-elect-lease-duration=60s  --kube-cloud-controller-manager-arg=leader-elect-renew-deadline=30s  --kube-cloud-controller-manager-arg=leader-elect-lease-duration=60s  --kube-scheduler-arg=leader-elect-renew-deadline=30s  --kube-scheduler-arg=leader-elect-lease-duration=60s"
{{ end }}
Environment="K3S_EXTRA_ARGS=--node-name={{ .HostName }}  --node-ip={{ .NodeIP }}  --pause-image={{ .PauseImage }} {{ range .KubeletArgs }} --kubelet-arg={{ . }}{{- end }} {{ range .KubeProxyArgs }} --kube-proxy-arg={{ . }}{{- end }} {{ .Container }} {{ .CgroupDriver }}"

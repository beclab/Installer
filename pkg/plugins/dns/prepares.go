...
func (c *CoreDNSExist) PreCheck(runtime connector.Runtime) (bool, error) {
	_, err := runtime.GetRunner().SudoCmdExt("/usr/local/bin/kubectl get svc -n kube-system coredns", false, false)
	if err != nil {
...
func (n *NodeLocalDNSConfigMapNotExist) PreCheck(runtime connector.Runtime) (bool, error) {
	if _, err := runtime.GetRunner().SudoCmdExt("/usr/local/bin/kubectl get cm -n kube-system nodelocaldns", false, false); err != nil {
		if strings.Contains(err.Error(), "NotFound") {
			return true, nil
		}
		return false, err
